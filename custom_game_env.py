...
class CustomGameEnv(gym.Env):
	...
	def update_game(self, action):
		...
		for opponent in self.opponents:
			...
			if result:
				...
				# Füge die alten und neuen Positionen zur Liste der aktualisierten Rechtecke hinzu
				self.updated_rects.append(pygame.Rect(old_pos[0], old_pos[1], tile_size, tile_size))
				self.updated_rects.append(pygame.Rect(new_pos[0], new_pos[1], tile_size, tile_size))
			...
			for bullet in list(opponent.bullets):
				...
				if bullet.check_bullet_collision(self.player_position):
					if RENDER:
						...
						print("playerposition", self.player_position)
	...
	def redraw_updated_areas(self):
		for rect in self.updated_rects:
			# Neuzeichnen der Bereiche in rect
